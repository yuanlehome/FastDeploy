...
def _get_inject_token_ids(limit_strategy: str, think_end_id: int, line_break_id: int = None) -> "paddle.Tensor":
    """Compute inject_token_ids tensor from strategy parameters."""
    if limit_strategy == "</think>":
        return paddle.to_tensor([think_end_id], dtype="int64")
    elif limit_strategy == "\n</think>\n\n":
        assert line_break_id is not None and line_break_id > 0
        return paddle.to_tensor([line_break_id, think_end_id, line_break_id, line_break_id], dtype="int64")
    else:
...
def post_process_normal(
    ...
    if think_end_id > 0:
        inject_token_ids = share_inputs.get(
            "inject_token_ids",
            _get_inject_token_ids(envs.FD_LIMIT_THINKING_CONTENT_TRUNCATE_STR, think_end_id, line_break_id if line_break_id > 0 else None),
        )
        max_reply_lens = share_inputs.get("max_reply_lens", paddle.full_like(share_inputs["max_think_lens"], -1))
        splitwise_role_is_decode = share_inputs.get("splitwise_role_is_decode", False)
        limit_thinking_content_length(
            ...
            splitwise_role_is_decode=splitwise_role_is_decode,
        )
    # 1. Set stop value
    paddle.assign(
        paddle.where(
